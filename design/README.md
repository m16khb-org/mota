...
다만 `apps/web`의 코드는 아직 이전 시스템(루트 `DESIGN.md` 2·3절의 검정·백색·
라임)을 구현하고 있다. 정해진 것은 이 시스템이고 코드 이관이 남아 있는 상태이므로,
기존 화면을 읽을 때는 두 시스템이 색과 글자에서 어긋나 있다고 보면 된다. 전체
화면을 한꺼번에 이관하는 것은 별도 과제다.

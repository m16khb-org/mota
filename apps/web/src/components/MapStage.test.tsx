...
describe("MapStage responsive map", () => {
	...
	it("searches and saves subway stations on the existing map", async () => {
		const { container } = renderStage(true, "subway");
		...
		const candidate = await waitFor(() => {
			const result = container.querySelector<HTMLButtonElement>(
				".inline-map-result[data-line]",
			);
			if (result === null) {
				throw new Error("Inline subway candidate was not rendered");
			}
			return result;
		});
		expect(candidate).toHaveAttribute("data-line", "8호선");
		fireEvent.click(
			await screen.findByRole("button", {
				name: "후보 지하철역 천호",
			}),

...
describe("SubwayArrivalList", () => {
	...
	it("subtracts each train row's provider delay from its ETA", () => {
		vi.useFakeTimers();
		vi.setSystemTime(new Date("2026-08-20T03:10:30.000Z"));
		const baseArrival = arrivals[0];
		if (baseArrival === undefined) {
			throw new Error("Expected a subway arrival fixture.");
		}
				...
				arrivals={[
					{
						...baseArrival,
						seconds: 180,
						generatedAt: "2026-08-20T03:08:30.000Z",
						message: "3분 후",
					},
				]}
				loading={false}
				error={null}
				updatedAt="2026-08-20T03:10:30.000Z"
				onRefresh={vi.fn()}
			/>,
		);
	...
	it("defaults to the line selected at a transfer station", () => {
		...
		const transferArrivals: readonly SubwayArrival[] = [
			{
				...baseArrival,
				id: "1005-상행-방화행",
				subwayId: "1005",
		...
		expect(screen.getByRole("tab", { name: "8호선 하행" })).toHaveAttribute(
			"aria-selected",
			"true",
		);
		expect(screen.getByText("모란행")).toBeInTheDocument();
		expect(screen.queryByText("방화행")).not.toBeInTheDocument();
	});

...
describe("apiStationName", () => {
	it("maps short OSM names to parenthesized official API names", () => {
		...
		expect(apiStationName("서울역")).toBe("서울");
		expect(apiStationName("응암")).toBe("응암순환(상선)");
		expect(apiStationName("공릉")).toBe("공릉(서울산업대입구)");
		expect(apiStationName("남한산성입구")).toBe(
			"남한산성입구(성남법원, 검찰청)",
		);
		expect(apiStationName("대모산입구")).toBe("대모산");
		expect(apiStationName("몽촌토성")).toBe("몽촌토성(평화의문)");
...
describe("normalizeSubwayArrivals", () => {
	it("maps upstream rows to line names, ETAs, and train status", () => {
		...
		expect(first?.generatedAt).toBe("2026-08-20T03:10:20.000Z");
	...
	it("distinguishes an unavailable zero ETA from a train arriving now", () => {
		...
		expect(secondsByMessage.get("전역 도착")).toBeNull();
	});

	it("excludes a train that already departed the requested station", () => {
		const { arrivals } = normalizeSubwayArrivals({
			errorMessage: { code: "INFO-000", message: "정상 처리되었습니다." },
			realtimeArrivalList: [
				{
		...
		expect(arrivals[0]?.message).toBe("전역 출발");
	});

	it("deduplicates repeated rows for the same operating train", () => {
		const repeatedTrain = {
			subwayId: "1008",
			updnLine: "하행",
			trainLineNm: "모란행",
		...
		const { arrivals } = normalizeSubwayArrivals({
			errorMessage: { code: "INFO-000", message: "정상 처리되었습니다." },
			realtimeArrivalList: [repeatedTrain, repeatedTrain],
		});
